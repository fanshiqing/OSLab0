...
#define CHECK_PER_SECOND 10 //检测食物是否被吃掉的频率
#define UPDATE_PER_SECOND 100  //刷新的时间间隔
	...
	while (TRUE) {
		...
		//while (now < target) { 
			...
			if (now % (HZ / UPDATE_PER_SECOND) == 0) {
				update_snake_pos();
				JudgeDie();//每次刷新蛇的位置后判断其是否死亡
				JudgeEat();//每移动一次要判断蛇是否吃到食物
				create_new_food();
			}
